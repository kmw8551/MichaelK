...
class StateMachine:
    ...
    def traverse_TCP_states(self, traverse_list):
        try:
            ...
            raise Exception(".set_start() shoulbe called before .run , Mendatory") #start 지점 선정, 선정 안되어있을씨 예외처리(에러발생)
...
def CLOSED(state_list):
    ...
    #입력받은 상태 값이 있을 경우에 동작이 가능, 그렇지 않을 경우에는 바로 프로세스 종료 (밑의 else 구문)으로 넘어가야한다. 
    if len(state_list) > 0:
        
        #pop을 사용하여 뺀 값은 리스트 형식이 아니라 문자처럼 나오기에 []로 한번 더 묶어주어서 두개의 리스트가 존재하는 것처럼 해야 편하다.
